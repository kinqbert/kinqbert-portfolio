...
class RetroEffectImpl extends Effect {
  ...
  constructor({ colorNum = 4, pixelSize = 2 }: Partial<RetroEffectProps> = {}) {
...
function RetroEffect({ colorNum, pixelSize }: RetroEffectProps) {
  const WrappedEffect = useMemo(() => wrapEffect(RetroEffectImpl), []);
  return <WrappedEffect colorNum={colorNum} pixelSize={pixelSize} />;
}
...
function DitheredWaves({
  ...
  useFrame(({ clock }) => {
    ...
    if (enableMouseInteraction) current.mousePos.value.copy(mouseRef.current);
  });
